...
using UnityEngine;
using UnityEngine.EventSystems;//判断鼠标是否在UI上
    ...
    void Update()
    {
        if (Input.GetMouseButton(0))
        {
            if (!EventSystem.current.IsPointerOverGameObject())
            {
                ...
                Vector3 temp = Camera.main.ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y, transform.position.z+10));
